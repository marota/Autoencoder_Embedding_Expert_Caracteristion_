...
#un modèle CVAE ou l'on peut personnaliser les distributions de l'espace latent
class CVAE_stat(BaseModel):
    def __init__(self, input_dim=96, cond_dim=12, z_dim=2, e_dims=[24], d_dims=[24], beta=1,distribution='Gaussian', mixture=False, embeddingBeforeLatent=False,pDropout=0.0, verbose=True,is_L2_Loss=True,has_skip=True,has_BN=1,**kwargs):
        super().__init__(**kwargs)
        self.input_dim = input_dim
        self.cond_dim = cond_dim
        self.z_dim = z_dim
    ...
    def build_model(self):
        ...
        # Encoding
        if self.mixture:
            z_mu_1, z_log_sigma_1 = self.encoder([x_true, cond_true])
            z_mu_2, z_log_sigma_2 = self.encoder([x_true, cond_true])
        else:
        	z_mu, z_log_sigma = self.encoder([x_true, cond_true])
        #self.latent=Lambda(lambda x:x,'latent')(z_mu)
        ...
        self.latent=Model(inputs=[x_inputs], outputs=[x], name='z_mu_output')
        if self.mixture:
        	ZMU1 = self.latent(z_mu_1)
        	ZMU2 = self.latent(z_mu_2)
        else:
        	ZMU=self.latent(z_mu)
        ...
        def sample_z(args):
        	if self.mixture:
        	    mu1, log_sigma1, mu2, log_sigma2 = args
        	    if self.distribution == 'Gaussian':
        	    	intra_dim = K.shape(mu1)[0]
        	    	unif = K.random_uniform(shape = (intra_dim, 1), minval=0.0, maxval=1.0)
        	    	eps_1 = K.random_normal(shape=(intra_dim, self.z_dim), mean=0., stddev=1.)
        	    	norm_1 = mu1 + K.exp(log_sigma1 / 2) * eps_1
        	    	eps_2 = K.random_normal(shape=(intra_dim, self.z_dim), mean=0., stddev=1.)
        	    	norm_2 = mu2 + K.exp(log_sigma2 / 2) * eps_2
        	    	treshold = 0.5
        	    	ech = Lambda(lambda unif,ech_1,ech2: norm_1*K.greater_equal(unif,treshold)+norm_2*K.less(unif, treshold),name='z_mu')(unif, norm_1,norm_2)
        	    	return ech
        ...
        if self.mixture:
        	self.dist_args=[z_mu_1, z_log_sigma_1, z_mu_2, z_log_sigma_2]
        else:
        	self.dist_args=[z_mu, z_log_sigma]
        	
        z = Lambda(sample_z, name='sample_z')(inputs = self.dist_args)
        ...
        # Defining loss
        vae_loss, recon_loss, kl_loss = self.build_loss(self.dist_args,weight=self.beta)
    ...
    def build_loss(self, args,weight=0):
        """

        :return:
        """
        def gaussian_pdf(z, mu, log_sigma):
        	return 1./K.sqrt(K.sum(K.exp(log_sigma_1), axis=-1))*K.exp(-0,5*K.sum(K.square((z-mu)/K.exp(log_sigma)), axis=-1))
        	
        def mixture_pdf(z, mu1, log_sigma1, mu2, log_sigma2, theta=0.5):
        	return theta*gaussian_pdf(z, mu1, log_sigma1) + (1.-theta)*gaussian_pdf(z, mu2, log_sigma2)
			 
        def kl_loss(y_true, y_pred):
        	if self.mixture:
        		mu1, log_sigma1, mu2, log_sigma2 = args
        		n_gen = 10**4
        		unif = K.random_uniform(shape = (n_gen, 1), minval=0.0, maxval=1.0)
		        eps_1 = K.random_normal(shape=(n_gen, self.z_dim), mean=0., stddev=1.)
		        norm_1 = mu1 + K.exp(log_sigma1 / 2) * eps_1
		        eps_2 = K.random_normal(shape=(n_gen, self.z_dim), mean=0., stddev=1.)
		        norm_2 = mu2 + K.exp(log_sigma2 / 2) * eps_2
		        
		        treshold = 0.5
		        z_ech = Lambda(lambda unif,ech_1,ech2: norm_1*K.greater_equal(unif,treshold)+norm_2*K.less(unif, treshold),name='z_mu')(unif, norm_1,norm_2)
		        
		        return 0.5 * K.sum(0.5*(K.exp(log_sigma1) + K.square(mu1)) + 0.5*(K.exp(log_sigma2) + K.square(mu2)), axis=-1) + K.sum(K.log(mixture_pdf(z_ech, mu1, log_sigma1, mu2, log_sigma2)), axis=-1)/n_gen
...
class CVAE_stat_emb(CVAE_stat):
    """
    Improvement of CVAE that encode the temperature as a condition with personnalized latent distribution
    """
    def __init__(self, to_emb_dim=96, cond_pre_dim=12, emb_dims=[2], emb_to_z_dim=[3],is_emb_Enc_equal_emb_Dec=True, **kwargs):
    ...
    def build_model(self):
        ...
        # Encoding
        if self.mixture:
            z_mu_1, z_log_sigma_1 = self.encoder([x_true, cond_true_enc])
            z_mu_2, z_log_sigma_2 = self.encoder([x_true, cond_true_enc])
        else:
        	z_mu, z_log_sigma = self.encoder([x_true, cond_true_enc])
        #self.latent=Model(inputs=[x_inputs], outputs=[z_mu], name='z_mu')
        #self.latent=Lambda(lambda x: x,name='latent')(z_mu)
