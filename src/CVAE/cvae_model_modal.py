...
class CVAE_modal(BaseModel):
    ...
    def build_model(self):
        ...
        # Decoding
        x_hat= self.decoder([z, cond_true, z_y])
    ...
    def build_decoder(self):
        ...
        y_inputs = Input(shape=(self.n_clusters,), name='dec_y')
        
        if(self.cond_dim>=1):
            cond_inputs = Input(shape=(self.cond_dim,), name='dec_cond')
            x = concatenate([x_inputs, cond_inputs, y_inputs], name='dec_input')#BatchNormalization()(cond_inputs) or not?
        else:
            cond_inputs = Input(shape=(0,), name='dec_cond')
            x = concatenate([x_inputs, cond_inputs, y_inputs], name='dec_input')
        ...
        if(self.cond_dim>=1):
            return Model(inputs=[x_inputs, cond_inputs, y_inputs], outputs=output, name='decoder')
        else:
            return Model(inputs=[x_inputs, cond_inputs, y_inputs], outputs=output, name='decoder')
    ...
    def build_loss_mixture(self, z_mix_mu, z_mix_log_sigma, z_y,weight=0):
        ...
        def kl_loss(y_true, y_pred):
            ...
            prior_mu = K.arange(start=-1., stop=1.001, step= 2/(self.n_clusters-1), dtype='float32') * self.n_clusters
